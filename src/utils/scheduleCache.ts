...
const cache = createLocalCache<ScheduleLocalCache>('schedule-cache-v2');

export const readScheduleCache = cache.read;
export const writeScheduleCache = cache.write;
// 로그아웃 시 호출 — 같이 쓰는 컴퓨터에서 로그아웃 후에도 일정 내용이 localStorage에 남아있지 않도록
export const clearScheduleCache = cache.clear;
export const isSameScheduleMeta = isSameMeta<ScheduleMeta>;
